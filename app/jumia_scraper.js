...
(async () => {
  ...
  for (let p = 1; p <= parseInt(maxPages || 1); p++) {
    ...
    try {
      ...
      const productLinks = await page.$$eval('a[data-testid="product-card-title"]', els =>
        els.map(a => a.href)
      );
      ...
      for (const detailUrl of uniqueLinks) {
        ...
        try {
          ...
          const title = await detail.$eval("h1", el => el.textContent.trim()).catch(() => null);
          const price = await detail.$eval("span[data-testid='product-price']", el => el.textContent.trim()).catch(() => null);
          const seller = await detail.$eval(".seller-name", el => el.textContent.trim()).catch(() => null);

          const bodyText = await detail.evaluate(() => document.body.innerText);
